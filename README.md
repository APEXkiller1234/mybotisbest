...
| `!cart`       | Posts a button; clicking reveals the Jace's links + service card **ephemerally**. | 🛡️ Admin only |
...
- `!mmpanel` sends the Middleman Service embed. Clicking **🎟️ Request
...
- `!mmtos` posts the little "Get a Manual Middleman here -> #channels" line
  ...
  Middleman Terms of Service **ephemerally** (only the clicker sees it).
- `!cart` posts a **View Links** button; clicking it reveals the website +
  invite links, banner and Jace's Middleman Service card **ephemerally**.
...
| `cart`       | `buttonLabel`        | Label of the `!cart` button (default `View Links`).                 |
| `cart`       | `buttonEmoji`        | Emoji of the `!cart` button (default 🔗).                            |
